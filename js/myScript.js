...
function generateField (row, col) {
    ...
    function addNeighbor (currentNodeCost, i, j) {
        if ((i < 0 || i >=  grid.sizeY) || (j < 0 || j >= grid.sizeX))
            return;
        if ((flowfield[i][j].cost + currentNodeCost < flowfield[i][j].totalCost) ||
            (flowfield[i][j].cost + currentNodeCost <=
                flowfield[i][j].totalCost && Math.round(Math.random()))) {
    ...
    while(queue.length != 0) {
        ...
        addNeighbor (sumCost - currentNode.cost + currentNode.cost * 1.4,
            currentNode.posY - 1, currentNode.posX +1 );
    ...
    for (var i = 0; i < grid.sizeY; i++)
        for (var j = 0; j < grid.sizeX; j++) {
            if (flowfield[i][j].goesTo){
                //flowfield[i][j].rect.fillColor = 'blue';
                drawFlow(flowfield[i][j], flowfield[i][j].goesTo);
            }
        }
}
...
function onKeyDown(event) {
    ...
    currentWeight = ( Math.pow(parseInt(event.key), 1.5) || 10000) ;
